...
const ChatItem: FC<ChatItemProps> = ({ room, onSelect, isActive }) => {
  return (
    <ListItem onClick={onSelect} sx={{ height: '100%', width: '100%', cursor: 'pointer', backgroundColor: isActive ? '#002C87' : 'transparent', borderBottom: '#002C87 solid 1px' }}>
      <ListItemText sx={{ color: '#fff' }} primary={<Typography variant='h6' sx={{ fontSize: '0.875rem' }}>{room.name}</Typography>} secondary={<Typography noWrap sx={{ fontSize: '0.85rem', fontWeight: '200' }}>{room.messages[0]?.message ?? ''}</Typography>} />
